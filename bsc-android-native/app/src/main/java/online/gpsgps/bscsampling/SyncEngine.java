...
final class SyncEngine {
  ...
  Result run(){Result out=new Result();if(!Util.online(c)){out.message="当前离线，数据已保存在手机";d.log("info","SYNC_SKIP_OFFLINE",detail());return out;}d.log("info","SYNC_BEGIN",detail());
    ...
    for(JSONObject s:d.pendingSchedules()){try{a.schedule(s.optLong("taskId"),s.optString("plannedTime",""));d.scheduleDone(s.optLong("taskId"));}catch(Exception e){fail("SYNC_SCHEDULE task="+s.optLong("taskId"),e);out.errors++;}}
