...
final class Api {
  ...
  JSONObject sync()throws Exception{return auth("GET","/api/v1/mobile/sync",null);} JSONObject schedule(long id,String time)throws Exception{return auth("POST","/api/v1/mobile/tasks/"+id+"/schedule",new JSONObject().put("plannedTime",time==null?"":time));} JSONObject start(long id,double lat,double lon,double acc)throws Exception{return auth("POST","/api/v1/mobile/tasks/"+id+"/start",new JSONObject().put("latitude",lat).put("longitude",lon).put("accuracyM",acc));}
  JSONObject tracks(long id,JSONArray a)throws Exception{return auth("POST","/api/v1/mobile/journeys/"+id+"/track",new JSONObject().put("points",a));} JSONObject live(long id,JSONObject b)throws Exception{return auth("POST","/api/v1/mobile/tasks/"+id+"/live",b);} JSONObject record(long id,JSONObject b)throws Exception{return auth("POST","/api/v1/mobile/tasks/"+id+"/record",b);} JSONObject complete(long id)throws Exception{return auth("POST","/api/v1/mobile/journeys/"+id+"/complete",new JSONObject());} JSONObject interrupted(long id)throws Exception{return auth("POST","/api/v1/mobile/journeys/"+id+"/interrupted",new JSONObject());} JSONObject logs(JSONArray a)throws Exception{return auth("POST","/api/v1/mobile/logs",new JSONObject().put("logs",a));} JSONObject deviceStatus(int pendingTracks,int pendingRecords,boolean syncCompleted)throws Exception{return auth("POST","/api/v1/mobile/device-status",new JSONObject().put("pendingTrackCount",pendingTracks).put("pendingRecordCount",pendingRecords).put("syncCompleted",syncCompleted).put("appVersion",BuildConfig.VERSION_NAME));}
