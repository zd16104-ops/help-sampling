...
双人协作：每个任务可指定主采样人和一名备用采样人，两人都能看到同一任务和合并进度，但同一时刻只有“当前采样人”能操作采样。备用人联网后可选择原因并输入确认码 `1234` 直接接管；接管有 60 秒冷却且允许以后再次接管。接管前已扫码、已拍照、轨迹和离线补传证据全部保留，只有当前采样权版本能形成最终记录。
