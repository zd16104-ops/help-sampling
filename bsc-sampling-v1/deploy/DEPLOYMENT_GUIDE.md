...
- 从旧版升级到 `1.6.0` 前，先执行 `npm run backup` 并保留整份 `data\v1\`；替换程序文件后首次启动会幂等增加双人采样、接管事件和进度表，不会修改既有任务编号、二维码、照片或采样记录。
...
- [ ] 下发一项主/备用双人任务；备用端能查看进度，输入 `1234` 接管后主端转为只读，管理站能合并显示两人的轨迹和证据
- [ ] 从任务列表选择已下发任务补打标签；取消任务不可打印，单次批量不超过 500 项
