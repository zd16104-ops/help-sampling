...
> 文档状态：V1 开发基线  
> 更新日期：2026-08-26  
> 正式域名：`https://bsc.gpsgps.online`  
> 适用范围：巴松措及周边约 30 km，采样人员不超过 10 人

---

## 1. 文档目的

本文档是系统设计、开发、测试、部署和验收的唯一 V1 基线，用于：

1. 把已确认的业务规则固定下来，防止后续开发偏离。
2. 明确 Android APP、Windows 服务器、管理员网站之间的责任边界。
3. 让新的 AI 或开发人员可以继续开发，不必重新询问已经确认的问题。
4. 明确“已实现”、“部分实现”和“待实现”，不把原型当成可上线产品。

## 2. 项目背景与目标

采样点距离管理人员较远，采样日期不固定，需要由当地村民完成采样。系统的核心不是一般的表单填报，而是在操作尽可能简单的前提下，形成一组可以人工复核的采样证据。

V1 的三个优先目标：

1. **定位**：村民能看到自己和任务点，系统能记录目标距离与轨迹。
2. **真实性**：结合服务器任务、瓶子二维码、现场相机照片、环境背景、GPS 与连续轨迹进行复核。
3. **数据回传**：无网可完成采样，联网后自动且可重试地上传，服务器确认前不丢数据。

技术无法单独证明一次采样百分之百真实。V1 的正确定位是“增加造假成本，保留原始证据，自动标记风险，交由管理员判断”。

## 3. 用户与权限

### 3.1 管理员

管理员数量不限，V1 先支持统一管理账号，正式服务器必须使用强密码，建议启用 TOTP 动态验证码。

管理员可以：

- 管理项目和采样点。
- 通过地图点选、经纬度输入或 Excel/CSV 导入点位。
- 上传每个点位的现场参考图。
- 临时创建采样任务、分配采样员、生成瓶子编号和二维码。
- 查看执行中位置、轨迹、现场照片和自动风险项。
- 作出审核通过、标记可疑、退回重采、无法采样等决定。
- 导出数据、照片、轨迹、二维码对应表和审核记录。
- 查看 APP 上传的诊断日志。

管理员不能修改原始采样记录；只能追加审核意见或标记作废。

### 3.2 采样员（村民）

- 人数不超过 10 人。
- 首次使用扫描管理员生成的设备激活二维码。
- 以后打开 APP 只输入 4 位 PIN；已激活设备可离线登录。
- 只能查看自己的任务和上传状态。
- 不能自己创建任务、选择编号或修改目标坐标。
...
| 主测试机 | OPPO Find X7，Android 15，12 GB + 256 GB，有 SIM 卡 |
| 网络 | 4G、5G 或任意 Wi-Fi |
| 服务器 | Windows Server 2022，1 核 / 4 GB，现有 Nginx |
| 后端 | Node.js 22+，SQLite，照片文件目录 |
| 公网入口 | `https://bsc.gpsgps.online` |
| 坐标 | WGS84，数据库始终保存十进制纬度/经度 |
| 时间 | 客户端记录 ISO 8601 时间和时区；服务器额外记录接收时间 |
| 地图 | 授权的在线卫星图 + 管理员导入的 MBTiles 离线包 |
| 离线包 | 巴松措周边约 30 km，单包不超过 2 GB |

## 5. V1 范围

### 5.1 必须交付

- 原生 Android APP，不使用 WebView 包装网页。
- 管理员网站。
- Windows 原生 Node.js 服务，不要求 Docker、Hyper-V 或重启整台服务器。
- HTTPS 反向代理方案。
- 离线任务、离线轨迹、离线照片和自动补传。
- 二维码生成、A4 每页 40 枚标签。
- 审核、风险标记、数据导出、日志和备份。
...
- 从相册选照片。
- 视频上传。
- 对 Google 卫星图或其他禁止缓存的图源进行批量抓取。
- 用 GPS 单独作出“真实/造假”的自动结论。
...
低并发且不超过 10 人的场景下，SQLite + WAL 足够，可以避免再引入一套数据库服务。照片不以 Base64 长期保存在数据库中，数据库只保存路径、大小与 SHA-256。

## 7. 核心业务对象

### 7.1 项目 Project

必填字段：项目编码、项目名称、正式/测试标志、是否启用。

### 7.2 固定采样点 Site

| 字段 | 规则 |
|---|---|
| 排序编号 | Excel 中的“编号”，只用于显示排序 |
| 历史序号 | 如 `5`、`5.1`、`5.2`，可包含小数点，不得转成浮点后丢失格式 |
| 点位名称 | 给村民看的中文名称 |
| 纬度/经度 | WGS84 十进制数，纬度在前、经度在后 |
| 海拔 | 可选，米 |
| 样品类型 | R/T/S/P/Y/L 可多选 |
| 距离阈值 | 正常 30 m，异常 80 m，硬上限 300 m |
| 现场参考图 | 启用点位必填；显示山体、河岸、桥或其他明显参照物 |
| 采样说明 | 给村民的简短操作提示 |
| 风险提醒 | 如湿滑、落石、涨水、无法靠近 |
| 备注 | 保留历史调查信息，如“土，植无水” |
| 启用状态 | 禁用后不再创建新任务，历史数据保留 |

同一项目内历史序号唯一。纬经度完全一样的 9.5 与 9.6 仍是两个独立点位，系统只给出“坐标重复”提示，不自动合并。

### 7.3 采样任务 Task

固定点可以产生任意多次任务。任务包含计划日期、点位、采样员、样品类型、瓶号、二维码密钥、状态、锁定设备、共享行程、取消信息。

同一天、同一点、同一类型采两个样品时，必须创建两个任务和两个编号，可共享同一条前往轨迹。

### 7.4 行程 Journey 与轨迹 TrackPoint

一次“开始前往”产生一个行程。同一采样员、同一设备、同一点位未结束的多个任务复用该行程。每个轨迹点包含顺序号、时间、WGS84 坐标、精度、速度与模拟位置标志。

### 7.5 采样记录 Record

一个任务只允许一条主记录。断网冲突不覆盖原记录，而是作为冲突记录进入人工审核。每条记录有手机生成的 `client_record_id`，服务器以它保证重试上传不重复创建数据。
...
示例：`260822-R-12-01`、`260822-T-5.1-02`。
...
编号只能由服务器在管理员创建任务时产生。村民不能选号。顺序号在“日期 + 类型 + 点位”范围内从 01 递增。

### 8.2 瓶子二维码

二维码内容不能只是可猜测的文字编号，应同时包含任务编号和高强度随机密钥，示意：

```text
BSC-SAMPLE|260822-R-12-01|<random-token>
```

服务器校验任务和密钥是否一致。二维码已使用时，不允许另一次普通提交；断网冲突只能作为可疑记录保留。

### 8.3 标签打印

- A4 不干胶，5 列 × 8 行，每页 40 枚。
- 每枚必须同时显示二维码和完整文字编号。
- 辅助显示历史序号、中文样品类型、计划日期、项目简称。
- 打印页面使用固定毫米尺寸，隐藏导航栏，不依赖网页缩放。

## 9. Android APP 信息架构

底部只保留四个主入口：

1. **地图**：首页；显示本人位置、精度圆、全部任务点、点位距离。
2. **任务**：默认按直线距离从近到远，可看待采样、进行中和已完成。
3. **上传**：待上传、上传中、上传成功、失败原因与重试。
4. **我的**：账号、服务器、同步时间、权限检查、离线地图导入、诊断日志。

界面要求：简体中文、大字、大图标、一页一个主操作，尽量不让村民输入文字。正常流程的主按钮用绿色，危险/禁止用红色，待处理/可疑用橙色。

## 10. 村民端完整流程

### 10.1 首次激活

1. 管理员建立采样员账号和初始 4 位 PIN。
2. 管理站生成 24 小时有效的设备激活二维码。
3. APP 扫码取得服务器地址、账号和一次性激活密钥。
4. 输入 PIN，服务器校验通过后绑定设备 UUID。
5. APP 在本机以 PBKDF2 校验值保存离线 PIN，不明文保存 PIN。

### 10.2 打开 APP

1. 检查精确位置、后台位置、相机和通知权限。
2. 有网时同步任务；无网时继续显示 SQLite 中最后一次成功同步的任务。
3. 开始 WGS84 定位，显示“正在定位”、当前精度和任务距离。
4. 检查是否存在未完成行程；如有，显示“恢复采样”并恢复前台轨迹服务。

### 10.3 开始前往

1. 村民打开任务，先看现场参考图、历史序号、样品类型、距离、管理员说明与风险提醒。
2. 权限不完整时，禁止开始，用图文引导开启。
3. 点击“开始前往”。联网时由服务器锁定到当前设备；断网时允许开始，但标记 `offline_start_lock_unverified`。
4. 如开始时已在目标 300 m 内，提示“距离太近，轨迹证据较弱”，不阻止。
5. 每 10 秒记录一个轨迹点，显示常驻通知；任务执行中每 30 秒尝试发送实时位置。
6. APP 被系统结束或手机重启后，恢复行程并标记中断证据。
...
|---|---|---|
| 0–30 m | 允许正常采样 | 无距离风险 |
| >30–80 m | 允许，必须选择异常原因 | 可疑 |
| >80–300 m | 允许，必须选原因并明显警告 | 严重可疑 |
| >300 m | 禁止拍照与提交 | 硬限制 |

可选异常原因：无水、河岸无法靠近、道路中断、水位或地质危险、其他。“其他”必须填短说明。人身安全优先，APP 不得用文案鼓励村民冒险靠近。

GPS 精度大于 40 m 不阻止采样，但自动标记可疑。定位暂时消失时，可继续操作但必须保留上一个有效定位时间，不得伪装成当前定位。

### 10.5 扫描瓶子

1. 到达后、取样前扫描。
2. 只接受当前任务的服务器二维码。
3. 连续解码失败 3 次或相机页停留超过 10 秒后，才显示“二维码损坏”。
4. 手动输入前必须选择标签浸水、标签脱落、二维码破损、相机无法对焦或其他。
5. 手输内容必须与服务器任务编号完全一致。
6. 手输记录自动标记“严重可疑”，不得由 APP 自动判定通过。
7. 手输时仍只拍一张，但损坏标签必须在照片中可见。

### 10.6 无水流程

现场无水时，村民选择“无水，无法采样”，不扫瓶子、不使用空瓶，只拍一张能看到干涸环境的现场照片。服务器记录为 `no_water`，管理员单独审核。

### 10.7 拍照

- 强制调用 APP 内 CameraX，不调用系统相册选择器。
- 只拍 1 张横屏照片，提交前可重拍，提交后不能替换。
- 正常水样照片中，装满水且贴有二维码的瓶子放在近处，约占画面 1/4；后方必须能识别河流、湖泊或实际采样环境。
- 界面显示瓶子框和三分线。为兼顾近处标签和远处环境，提示用户不要把瓶子贴到镜头，建议保持约 40–80 cm，后方环境占画面大部分。
- 不提供裁剪、滤镜、修图或添加外部图片的入口。

### 10.8 水印

手机立即生成带水印的 JPEG，长边不超过 2560 px，JPEG 质量约 90，目标 1–3 MB。原始像素只在本机生成水印期间存在，成功写出水印照片后删除临时原图。V1 按用户最终选择，只保存带水印图。

照片底部约 20% 使用暗色半透明底，文字至少包含：
...
服务器后续补齐的天气保存在结构化字段并显示于审核页，不覆盖手机上传的证据照片。如以后确实需要“补天气水印图”，只能生成新的展示副本，必须保留原证据图的 SHA-256。

### 10.9 提交与结束

1. 提交前显示照片、编号、点位、距离和异常标志的最终确认页。
2. 数据先以一个本地事务写入 SQLite，照片先持久化到 APP 私有文件夹，然后界面显示“已保存”。
3. 有网立即上传；无网显示“已保存，等待网络”。
4. 只有收到服务器幂等成功回应后才把本地状态设为已上传。
5. 服务器确认后立即停止该行程的轨迹和实时位置。同点还有未完成任务时，延迟结束共享行程。

## 11. 离线与同步设计

### 11.1 本地存储

SQLite 表：任务快照、行程、轨迹点、待上传记录、应用日志。照片保存在 APP 私有文件夹，数据库保存路径和状态。

未上传数据不设自动删除期限，一直保留到服务器确认成功。采样员不能在界面中删除待上传记录。

### 11.2 同步触发

- APP 打开时。
- 用户点击“立即同步”。
- WorkManager 约每 15 分钟检查，要求有可用网络。
- 完成一次采样后。
- 网络恢复后由系统调度，不依赖 APP 一直在前台。

### 11.3 上传顺序

1. 取得/刷新设备令牌。
2. 上传未同步的轨迹点，每批最多 1000 点。
3. 上传采样记录和照片。
4. 服务器回复记录 ID 后标记本地成功。
5. 上传未发送的诊断日志。
6. 拉取服务器最新任务，与本地任务合并。

重试采用指数退避。HTTP 4xx 业务拒绝应在界面明确显示中文原因；超时、断网和 5xx 保留本地数据并继续重试。
...
submitted 已提交/待审核
   ...
   ├─ suspicious 可疑，等待处理
   ├─ no_water 无法采样
   └─ rejected 退回重采 → 新建重采版本
```

任务取消不删除数据。手机断网时已完成的取消任务仍可上传，服务器附加 `task_canceled` 风险标志。第二天完成计划任务时附加 `late_sampling`。

## 13. 真实性与风险规则

### 13.1 证据层级

1. **主证据**：现场照片中的瓶子/标签与背景水体、山体、河岸等环境。
2. **身份证据**：管理员任务、绑定设备、首设备锁、唯一二维码。
3. **位置证据**：拍摄坐标、精度、目标距离、连续轨迹。
4. **时间证据**：任务日期、行程时间、拍摄时间、服务器接收时间。
5. **交叉证据**：照片 SHA-256、相同图片检查、模拟位置标志、天气与场景人工对照。

### 13.2 自动风险标志

| 代码 | 含义 | 建议级别 |
|---|---|---|
| `distance_30_80m` | 距目标 30–80 m | 可疑 |
| `distance_80_300m` | 距目标 80–300 m | 严重可疑 |
| `gps_accuracy_over_40m` | GPS 精度数值超过 40 m | 可疑 |
| `manual_bottle_code` | 二维码损坏后手输 | 严重可疑 |
| `mock_location` | Android 报告模拟位置 | 严重可疑 |
| `duplicate_photo` | 照片 SHA-256 与既有记录相同 | 严重可疑 |
| `offline_start_lock_unverified` | 断网开始，无法服务器锁定 | 可疑 |
| `weak_start_track` | 开始前往时已在 300 m 内 | 可疑 |
| `track_interrupted` | 轨迹服务中断后恢复 | 可疑 |
| `missing_track` | 提交时没有可用轨迹点 | 可疑 |
| `late_sampling` | 拍摄日期与计划日期不一致 | 可疑 |
| `task_canceled` | 任务已取消后收到离线记录 | 严重可疑 |
| `weather_pending` | 拍照时断网，天气待补充 | 信息提示 |

自动风险不等于造假。审核页必须用中性语言显示“需要复核的证据”，最终判断仍由管理员完成。

### 13.3 可选的轻量反作弊

V1 可以读取 Android `Location.isMock()` 标志，检查不可能的速度/跳点，并将异常发送到服务器。不尝试“绕过 root 或对抗专业伪装工具”，因为这会显著增加复杂度，仍无法形成绝对证明。

## 14. 地图与坐标

### 14.1 坐标规则

- 入库、计算距离、导出均使用 WGS84（EPSG:4326）。
- 地图底图的 Web Mercator 投影只用于显示，不改变业务坐标。
- 不将 GCJ-02/火星坐标当成 WGS84 存储。
- 距离用球面 Haversine 公式计算，并统一以米显示。

### 14.2 离线影像

- APP 使用 MapLibre Native 显示地图。
- 离线包格式优先 MBTiles，管理员在网页下载或制作后，通过数据线人工导入手机。
- 无离线包时，联网可使用合法的在线卫星影像；断网仍显示本人位置和任务点，不能因底图失败而阻断采样。
- 实际影像的下载和缓存必须遵守数据提供方授权。免费的 Sentinel-2 可用于大区域背景；点位附近的高精度影像需使用已购买或已获授权的数据。

### 14.3 地图交互

首页只显示：自己位置、采样点、距离、精度、缩放和回到本人位置。放大后应能达到屏幕比例尺 50 m。不开发导航路线。

## 15. 天气设计

- 联网拍照时，APP 根据坐标调用免费天气接口，最多等待数秒；天气接口失败不阻塞采样。
- 断网或超时时写“待补充”。
- 服务器按拍摄坐标和拍摄时间查询历史天气，写入服务器天气字段。
- 天气是辅助信息，不作为单独造假判定依据。
...
左侧最大层级为“项目”，项目下按**照片实际拍摄日期**归档。另外提供“待采样”入口，不将未执行任务混入拍摄日期。

点击日期后：

- 主区域显示卫星地图与当日采样点。
- 点击点位打开右侧详情，查看现场照片、参考照片、轨迹、实际/目标位置、风险标志和审核意见。
- 地图标记使用一致状态色：灰色待采样、橙色待审核、绿色已通过、红色可疑/退回。

### 16.2 点位管理

- 地图逐个点击添加，可在保存前拖动微调。
- 也可粘贴 WGS84 纬度/经度。
- 支持 Excel/CSV 批量导入，保留“编号”和“历史序号”两个独立字段。
- 点位编辑不能改写已产生的历史任务快照。
- 未上传现场参考图的点位默认不启用，不允许下发正式任务。

### 16.3 任务下发

- 选项：项目、采样日期、点位、采样员、一个或多个样品类型。
- 选中多类型时，为每个类型生成独立任务、编号和二维码。
- 支持在地图上多选点位后批量下发。
- 下发后可批量打印 40 枚/页的不干胶标签。
- 可取消未完成任务，但保留取消人、时间和原因。
...
所有业务接口使用 `/api/v1` 前缀，JSON 使用 UTF-8。正式 APP 只允许 `https://` 服务器地址。

### 17.1 管理员接口

| 方法 | 路径 | 作用 |
|---|---|---|
| POST | `/api/v1/admin/login` | 密码 + 可选 TOTP 登录 |
| GET | `/api/v1/admin/bootstrap` | 项目、采样员与概览 |
| GET/POST | `/api/v1/admin/sites` | 查询/新建点位 |
| PUT | `/api/v1/admin/sites/{id}` | 编辑点位，写审计日志 |
| POST | `/api/v1/admin/reference-images` | 上传并压缩现场参考图 |
| POST | `/api/v1/admin/villagers/{id}/activation` | 生成一次性激活二维码 |
| GET/POST | `/api/v1/admin/tasks` | 查询/创建任务 |
| POST | `/api/v1/admin/tasks/{id}/cancel` | 取消任务 |
| POST | `/api/v1/admin/tasks/{id}/unlock` | 人工解锁设备 |
| GET | `/api/v1/admin/labels` | 生成 40 枚/页打印页 |
| POST | `/api/v1/admin/records/{id}/review` | 追加审核状态和意见 |
| GET | `/api/v1/admin/exports/{format}` | Excel/CSV/GeoJSON/GPX/照片包 |
| GET | `/api/v1/admin/logs` | 查看 APP 诊断日志 |

### 17.2 Android 接口

| 方法 | 路径 | 作用 |
|---|---|---|
| POST | `/api/v1/mobile/activate` | 首次激活和绑定设备 |
| POST | `/api/v1/mobile/login` | 已激活设备联网登录 |
| GET | `/api/v1/mobile/sync` | 下载本人任务和服务器规则 |
| POST | `/api/v1/mobile/tasks/{id}/start` | 首设备锁定任务并创建/复用行程 |
| POST | `/api/v1/mobile/journeys/{id}/track` | 批量上传轨迹点 |
| POST | `/api/v1/mobile/tasks/{id}/live` | 更新执行中实时位置 |
| POST | `/api/v1/mobile/tasks/{id}/record` | 幂等上传采样记录与照片 |
| POST | `/api/v1/mobile/journeys/{id}/complete` | 结束行程 |
| POST | `/api/v1/mobile/logs` | 上传脱敏诊断日志 |
...
| 404 | 任务/点位不存在 |
| 413 | 照片或请求体过大 |
| 422 | 距离超 300 m、二维码不匹配等业务拒绝 |
| 423 | 任务已被另一设备锁定 |
| 500 | 服务器异常，APP 保留本地数据重试 |
...
|---|---|---|
| `projects` | 项目 | `code` 唯一 |
| `sites` | 固定点位 | `project_id + code` 唯一，软删除 |
| `villagers` | 采样员 | `username` 唯一，PIN 只存盐和哈希 |
| `devices` | 已激活手机 | `villager_id + device_uuid` 唯一 |
| `activation_codes` | 一次性激活码 | 只存 token SHA-256，有过期/使用时间 |
| `tasks` | 采样任务 | `sample_code`、`qr_token` 唯一 |
| `journeys` | 前往行程 | 关联采样员、设备、点位 |
| `track_points` | 轨迹 | `journey_id + sequence` 唯一 |
| `live_locations` | 最新实时位置 | 每任务一条最新记录 |
| `records` | 采样证据 | `client_record_id` 唯一；每任务最多一条主记录 |
| `audit_logs` | 管理审计 | 只追加，不在网站提供删除 |
| `app_logs` | APP 诊断 | 禁止包含 PIN、token、QR 密钥或照片数据 |
| `app_versions` | APK 版本 | 版本码唯一，保存 SHA-256 |

数据库启用 WAL、外键和 `busy_timeout`。所有多步骤写入（如任务批量生成、行程创建与锁定）必须使用事务。

## 19. 照片存储与完整性

- 服务器目录建议：`data/v1/uploads/<project-id>/<sample-code>-<client-id>.jpg`。
- 服务器接收后校验 JPEG，自动旋转方向，限制长边 2560 px，保存后计算 SHA-256。
- 在数据库写入成功前，采用临时文件 + 原子重命名，防止宕机产生半张照片。
- 已审核记录的照片不得被同名覆盖。
- 按当前决定，服务器长期保留有水印的最高质量图；不另存无水印原图。

## 20. 安全设计

### 20.1 传输安全

- 正式站只开放 HTTPS 443，HTTP 80 只用于跳转和 Let’s Encrypt 验证。
- Node.js 只监听 `127.0.0.1:3100`，不直接暴露到公网。
- Nginx 限制请求体大小、连接超时和上传速率。
- Android 正式构建禁止明文 HTTP，不再出现 WebView Mixed Content 问题。

### 20.2 身份安全

- 4 位 PIN 的强度有限，必须和已激活设备绑定联合使用。
- 服务器 PIN 使用独立随机盐 + PBKDF2-SHA256，不存明文。
- 激活码一次性使用，默认 24 小时过期。
- 移动令牌带设备 ID 并有到期时间，服务器每次检查设备未停用。
- 登录、激活和 PIN 错误需增加速率限制和短时锁定。

### 20.3 数据最小化

实时位置只在任务“开始前往”到“完成采样”之间收集，完成后立即停止。不用于工作以外的持续监控。在采样员培训中必须明确说明采集内容、目的和时段。

## 21. 诊断日志

APP 日志至少包含：版本、Android 版本、设备型号、时间、网络类型、同步阶段、HTTP 状态、异常类型、本地队列数量和权限状态。

不得记录：管理密码、村民 PIN、完整 Bearer Token、激活密钥、二维码随机密钥、照片 Base64。

日志功能：

- 村民可一键复制/分享。
- 联网时自动上传到 `/api/v1/mobile/logs`。
- 管理员可按设备、人员、时间和级别查询。
- 严重错误保留堆栈的脱敏摘要，不将整个本地数据库上传。

## 22. 开源复用决策

| 能力 | 采用/参考 | 许可证 | 决策 |
|---|---|---|---|
| 离线优先流程 | ODK Collect | Apache-2.0 | 参考本地落盘、服务器确认后删除的可靠性思路 |
| 地图 | MapLibre Native 11.11 | BSD-2-Clause | 直接用于原生地图与离线图 |
| 野外离线交互 | QField | GPL-2.0+ | 只参考交互，不复制代码 |
| 轨迹 | Traccar Client | Apache-2.0 | 参考前台服务、断网缓存与常驻通知 |
| 相机 | Android CameraX | Apache-2.0 | 预览、解码帧和拍照 |
| 二维码 | ZXing Core 3.5.4 | Apache-2.0 | 只用解码核心，不依赖外部扫码 APP |
| 后台重试 | Android WorkManager | Apache-2.0 | 持久化同步和有网约束 |
| HTTP | OkHttp 4.12 | Apache-2.0 | HTTPS API，连接/读写超时 |
| 管理地图 | Leaflet 1.9.4 | BSD-2-Clause | 本地托管，不依赖 CDN |
| 照片处理 | Android Bitmap + sharp | Apache-2.0 | 手机水印、服务器校验与限尺寸 |
| 二维码生成 | node-qrcode | MIT | 激活码与标签 |
| 天气 | Open-Meteo | 按 API 条款 | 拍照天气和历史天气补充 |

明确不采用已归档的 osmdroid 作为新项目基础；不采用 WebView 壳；V1 单张 1–3 MB 照片不引入 tus 分块协议，而使用幂等 ID + 整包重试。

引入新开源库前必须检查：维护状态、官方文档、许可证、Android 10/15 适配、依赖体积与无网行为。

## 23. Windows 服务器部署设计

### 23.1 部署原则

- 不安装 Docker Desktop、Hyper-V 或 WSL2，不重启已有业务。
- Node.js 使用独立目录，默认监听 `127.0.0.1:3100`。如 3100 已占用，先查明进程而不是重复启动；可为新版选 3101，但 Nginx 配置和 APP 域名仍保持一致。
- 正式访问不使用 `192.168.x.x`、`127.0.0.1` 或手工填端口；APP 固定使用 `https://bsc.gpsgps.online`。
- 使用已有 Nginx 新增独立 `server_name`，不覆盖 `gpsgps.online` 和 `auto.gpsgps.online` 的现有配置。

### 23.2 DNS 与 HTTPS

1. 在京东云 DNS 为 `bsc.gpsgps.online` 添加 A 记录，指向 Windows 服务器公网 IP。
2. 先用 DNS 查询和 HTTP 验证确认解析生效。
3. 使用 Windows 可用的 ACME 客户端自动申请 Let’s Encrypt 证书。
4. Nginx 443 转发到 Node.js，并正确传递 `Host`、`X-Forwarded-For`、`X-Forwarded-Proto`。
5. 验证证书自动续期任务，在到期前 30 天进行一次手工演练。

### 23.3 进程管理

建议使用 Windows 服务或 NSSM 管理 Node.js，配置：开机自启、异常重启、固定工作目录、独立日志目录。不使用需要一直打开的 CMD 窗口作为正式运行方式。

## 24. 备份、保留与容量

- 数据库每天备份。备份 SQLite 时使用在线 backup API 或在确保 WAL 一致性的条件下复制，不只随手复制主 `.sqlite` 文件。
- 照片每周做增量或完整备份。
- 备份必须至少有一份不在当前服务器磁盘。
- 剩余空间小于 10 GB 时告警，小于 5 GB 时禁止新的大文件导出但不得拒绝采样证据上传。
- 结构化记录、审计日志和有水印照片长期保留；系统不提供硬删除按钮。
- 每月至少进行一次从备份恢复到临时目录的演练，不能只检查备份文件“存在”。
...
- 激活 QR 与样品 QR 解析。
- 编号正则与历史序号 `5.1`、`9.5` 保留。
- Haversine 距离计算的 30/80/300 m 边界。
- PIN 本地校验不存明文。
- SQLite 任务合并、轨迹去重、上传队列状态迁移。
- 日志脱敏。
...
- 激活码过期、重放、设备停用。
- 首设备锁定与 12 小时过期。
- 30/80/300 m 边界和异常原因必填。
- 二维码不匹配、手输不匹配、二维码重复。
- `client_record_id` 重试幂等性。
- 重复照片 SHA-256 风险。
- 照片过大、非 JPEG、路径穿越拦截。
- 管理审核不改写原记录。
...
主测试设备为 OPPO Find X7 / Android 15，另选一台 Android 10–12 设备做最低版本兼容测试。

必测场景：

1. 首次激活，之后断网 PIN 登录。
2. 手机系统为简体中文，字体放大一档。
3. Wi-Fi、4G、5G 切换，弱网、飞行模式、中途恢复。
4. 离线 MBTiles 导入、地图缩放、无底图仍显示任务。
5. 前台轨迹运行 2 小时，息屏、锁屏、切换 APP，检查电量与断点。
6. 手机重启后恢复未完成行程。
7. 29 m、30 m、31 m、79 m、80 m、81 m、299 m、300 m、301 m 附近边界。
8. 正常扫码，不同任务码，3 次失败/10 秒后手输。
9. 横屏拍照、重拍、画面引导、无相册入口。
10. 两个同点任务共享行程。
11. 照片上传到 50% 时断网，恢复后不产生重复记录。
12. 取消任务断网提交、跨日提交、无水提交。
13. 复制日志和自动上传日志。

### 25.3 服务器测试

- 空闲、10 台设备同时同步、10 张照片连续上传。
- Node.js 异常退出后由服务自动拉起。
- Nginx 超时不会导致重复记录。
- SQLite WAL 在备份时可一致恢复。
- 磁盘低空间告警。
- 证书续期演练。
- 不影响现有 3000、8090、3306 等服务。

## 26. V1 验收标准

| 编号 | 验收项 | 通过条件 |
|---|---|---|
| A01 | 公网连接 | 手机使用 4G/5G 能通过 `https://bsc.gpsgps.online` 激活、同步、上传 |
| A02 | 离线登录 | 激活后开飞行模式，4 位 PIN 可进入 |
| A03 | 任务下发 | 管理员创建后 APP 打开/后台 15 分钟内可见 |
| A04 | 编号 | 同日同点两个任务生成不同尾号 |
| A05 | 扫码 | 错码被拒绝；3 次/10 秒后才允许损坏流程 |
| A06 | 相机 | APP 内只能实时拍一张，没有相册入口 |
| A07 | 水印 | 暗色底，编号、WGS84、距离、精度、时间、天气清晰可读 |
| A08 | 距离 | 30/80/300 m 规则与异常原因完全正确 |
| A09 | 轨迹 | 息屏后仍每约 10 秒记录，完成后立即停止 |
| A10 | 离线补传 | 断网完成任务，杀死 APP/重启手机后数据仍在，联网后上传 |
| A11 | 幂等 | 同一记录重试 5 次，服务器仍只有一条主记录 |
| A12 | 管理审核 | 可对照现场/参考照片和轨迹，审核不改写原记录 |
| A13 | 标签 | A4 实际打印 5×8，普通手机可扫，文字编号可读 |
| A14 | 导出 | Excel/CSV、GeoJSON、GPX、照片包与审核记录可打开且一致 |
| A15 | 日志 | APP 可复制/上传，管理端可查，不含密钥 |
| A16 | 备份 | 从备份在临时目录恢复并打开任意一条照片记录 |
...
- 本文档定稿。
- 建立需求变更记录；修改 30/80/300 m、照片数量、编号等核心规则必须显式更新文档。

### 阶段 1：可连通的原生纵向流程

- 服务器初始化、激活、登录、任务同步。
- Android 激活、PIN、任务列表、单任务扫码/拍照/上传。
- 管理员能看到照片并审核。
- 先在“手机附近测试项目”验证。

### 阶段 2：真实性与离线

- 30/80/300 m、GPS 精度、模拟位置、轨迹。
- 本地队列、WorkManager、幂等重试。
- 同点共享行程、首设备锁、无水/手输/取消/跨日。

### 阶段 3：地图与管理工具

- 在线卫星图、MBTiles 导入、任务点和定位。
- Excel 导入、参考图、批量任务、40 枚/页标签。
- 照片/轨迹审核、导出、日志。

### 阶段 4：上线准备

- Windows 服务、Nginx、DNS、HTTPS、备份和容量告警。
- 正式 Android 签名，保存 keystore 离线备份和 APK SHA-256。
- OPPO Find X7 现场演练，小批量真实采样，再正式启用。

## 28. 当前代码状态（2026-08-26）

### 28.1 已写入源码

- Android 原生工程、激活/PIN 基础、SQLite 任务/行程/轨迹/记录/日志表。
- CameraX 扫码、ZXing 解码、CameraX 拍照、横屏构图引导和暗色水印。
- 前台轨迹服务、WorkManager 定期同步和重启恢复基础。
- MapLibre 地图、任务点与 MBTiles 导入基础。
- Node.js `/api/v1` 数据库、激活、任务、锁定、轨迹、记录、审核和日志基础接口。
...
1. 点位现场参考图由"启用必填"改为**可选（建议上传）**：服务器不再因无参考图强制停用点位；CSV 导入点位默认启用。
2. 点位 WGS84 坐标输入精度放宽到 8 位小数：输入框改为 `step=any` 并增加纬度 ±90/经度 ±180 范围校验，修复浏览器把 8 位小数坐标误判为"无效值"；所有对话框的取消/× 按钮增加 `formnovalidate`，字段无效时也能正常关闭对话框。
3. 左侧"采样日期"改为自动归档：待采样任务的计划日期 ∪ 已提交记录的拍摄日期；下发任务后该计划日期自动出现并切换、地图立即显示新任务；对日期本身不做增删改，提供任务级**改期**（编号按新日期重新生成 + 更换二维码密钥，旧标签作废需重印）与取消。
4. 项目支持增删改查：新建、编辑（含启用/停用）、删除（已有任务数据时拒绝删除，只能停用）。
5. 地图采样点名称常显（永久标签）；任务下发对话框增加"全选/全不选"。
6. 同一点位的多个任务不再重叠成一个点：按点位分组围绕中心展开，首标记显示"名称 ×数量"，悬停显示编号；地图标记总数与任务数一致。
7. 诊断日志全面增强（支持远程排障）：APP 日志带结构化详情（网络类型/是否在线/权限状态/队列数量/行程ID/距离/同步阶段）；同步开始/结束/跳过离线均记录；HTTP 失败附加状态码；轨迹每 60 点记录进度；APP 启动/进入/开始采样/保存队列均有结构化日志；手机重启恢复行程记录 BOOT_RESUME/TRACK_RESUME 并通过新接口 `/api/v1/mobile/journeys/{id}/interrupted` 上报中断证据（对应 `track_interrupted` 风险标志）；日志写入前脱敏（PIN/令牌/激活密钥打码）。服务端新增日志筛选（级别/设备/采样员/时间区间）与 `logs.csv` 导出；管理站日志对话框支持筛选与一键导出。
8. **取消 PIN 码（核心规则变更，用户决定）**：扫码激活即激活并登录——激活二维码（管理员端一次性生成、24 小时有效、绑定设备）就是凭证，之后打开 APP 直接进入，不再要求输入 PIN 或再次登录。服务器 `/api/v1/mobile/activate` 与 `/login` 不再校验 PIN；Android 端移除 PIN 界面与离线 PIN 存储。安全由管理员端二维码受控 + 设备绑定 + 管理端可停用采样员（同步停用其设备）保障。管理站新增"采样员管理"：新建采样员（账号+姓名）、生成激活二维码、停用/启用。§10.1/§10.2/§20.2 中关于 PIN 的描述以本条为准。
9. 地图放大后空白处理：在线影像（Esri World Imagery）在部分区域/层级无数据或加载失败，网页端增加**自适应层级锁定**（某层级瓦片连续 5 次失败后停止请求更深层级，继续放大用已有层级放大显示，工具栏显示提示）；Android 端在线影像源限制 `maxzoom=17`（继续放大为放大显示，不再请求更深层级）。MBTiles 离线包不受影响。
10. 地图采样点标记由旋转大头针改为**水滴外形轮廓**（SVG 泪滴形状，白色描边 + 状态色填充，灰/橙/绿/红含义不变），图例、标签、交互等其他元素不变。
11. 顶栏 7 个功能按钮（导出CSV/GeoJSON/照片包/审计记录/导入Excel/CSV/下发采样任务/设置采样点）旁增加小"!"信息点，鼠标悬停显示该功能的详细说明气泡。
12. **登录与配置简化（用户决定）**：管理员登录为**仅密码**（TOTP 为可选增强，`adminTotpSecret` 留空即关闭）；部署包不再把 `config.example.json` 放进 `data\v1\`，该目录只保留启动自动生成的唯一 `config.json`，示例配置移到 `bsc-server\` 根目录，避免"示例+正式"两个配置文件并存造成混淆（此前线上 401 即因误把示例密码当正式密码）。
13. 管理站地图**右键空白处直接新增采样点**：`map.on('contextmenu')` 打开点位对话框并自动带入该位置坐标；同时修复"地图选点"流程 bug（对话框关闭的 close 事件异步触发把 `pickMode` 复位、`finishPick` 丢坐标导致地图跳到 (0,0)）。
14. 点位坐标输入改为**单一输入框**，严格解析用户习惯格式 `【WGS84】29.66579301°N，94.34286257°E`（含全角括号/逗号/°符号容错），隐藏的经纬度字段由 `parseCoords()` 同步填充；输出复制也统一为该格式。
...
- Android 尚未完成 OPPO 真机验收与真机弱网/重启/边界测试。
- 正式签名 APK 尚未完成。
- 公网部署**进行中**：`bsc.gpsgps.online` 已解析并运行本系统（管理员登录已实测修复 401/429），Windows 服务化/Nginx/HTTPS/备份计划任务需按 `deploy/DEPLOYMENT_GUIDE.md` 在服务器上确认到位；真机公网验收（手机 4G 走通激活→采样→上传全流程）尚未完成。

## 29. 已知风险与应对

| 风险 | 影响 | 应对 |
|---|---|---|
| Android 15/OPPO 后台限制 | 息屏后轨迹可被结束 | 前台服务 + 常驻通知 + 权限引导 + 真机 2 小时测试 |
| 高山地区 GPS 漂移 | 30 m 内可能误判 | 不以 GPS 单独否定；保留精度、轨迹和照片人工审核 |
| 无网多设备同时开始 | 服务器锁无法即时生效 | 允许保存，上传后保留冲突且严重可疑，不覆盖 |
| 高清离线卫星图授权 | 不能随意批量下载 | 使用授权数据；免费数据作大区域，购买/授权影像作点位高精度 |
| Windows 服务器只剩约 18 GB | 照片增长后磁盘满 | 每日监测、10 GB 告警、每周异机备份、定期容量报告 |
| 4 位 PIN 强度低 | 离线猜测 | 绑定设备、错误次数延时、服务器限速，管理员可停用设备 |
| 照片中近景瓶子与远景难同时清晰 | 标签或背景无法复核 | 瓶子保持 40–80 cm，约占 1/4，先真机检验自动对焦，提交前允许重拍 |

## 30. 代码和文档规则

- 原生 APP、服务器、网站分目录，不将老 WebView 原型代码混入正式 APP。
- 固定第三方依赖版本；升级前查发行说明、许可证并完整回归。
- 公开 API 保持 `/api/v1`，破坏性更改使用新版本路径。
- 错误信息对村民使用简短中文；技术详情写诊断日志，不在主界面显示堆栈。
- 所有用户可见字符使用 UTF-8，Windows `.bat` 避免将中文当成命令执行；正式脚本使用 ASCII 文件名和明确引号。
- 每完成一个里程碑，同步更新本文档的“当前代码状态”和验收记录。

## 31. 最终交付物

1. 已签名的 Android APK 和 SHA-256。
2. Android 完整源码、Gradle 版本与可重复构建说明。
3. Windows Node.js 服务器、管理员网站和数据库迁移。
4. 京东云 DNS、Nginx、Let’s Encrypt、Windows 服务的部署手册与可交给服务器 AI 的操作提示词。
5. 采样点 Excel 导入模板、A4 40 枚标签模板。
6. 村民图文使用指南和管理员手册。
7. 每日数据库备份、每周照片备份、恢复演练脚本。
8. 自动测试报告、OPPO Find X7 真机验收表和上线检查表。
9. 开源软件清单、版本、许可证和来源。
...
- 正式域名：`bsc.gpsgps.online`。
- 原生 Android，Android 10+，简体中文大图标。
- 首次激活二维码 + 日常 4 位 PIN 离线登录。
- 任务由管理员临时下发，同点可反复采样。
- 30 m 正常，30–80 m 可疑，80–300 m 严重可疑，超过 300 m 禁止。
- 轨迹 10 秒/点，实时位置 30 秒/次，完成后立即停止。
- 正常必须扫码；3 次失败或 10 秒后才允许损坏手输，严重可疑。
- 每任务一张横屏现场照片，无相册入口，暗色 WGS84 水印。
- 任务/轨迹/照片无网先落盘，成功上传前不自动删除。
- 巴松措周边约 30 km 授权离线卫星地图，不超过 2 GB。
- A4 不干胶每页 40 枚。
- 照片是主证据，GPS 和轨迹是辅助证据，最终由管理员审核。

<!-- BEGIN GENERATED SOURCE SNAPSHOT -->
...
SHA-256: `4eec6df529cfbf27a0691f92d8aad18f91b8fff11f9a49f3fc52858c656163e3`
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <application android:name=".SamplingApp" android:theme="@style/Theme.Bsc" android:label="巴松措采样"
        ...
        <activity android:name=".PhotoActivity" android:screenOrientation="landscape" android:exported="false"/>
...
SHA-256: `405e9d3e14360556e7b74d1c19404a7811c79be016cfead18796a415bb7c9193`
...
final class Store extends SQLiteOpenHelper {
  ...
  void tasks(JSONArray array){SQLiteDatabase d=getWritableDatabase();d.beginTransaction();try{for(int i=0;i<array.length();i++){JSONObject o=array.optJSONObject(i);if(o==null)continue;ContentValues v=new ContentValues();v.put("id",o.optLong("id"));v.put("json",o.toString());v.put("updated_at",Util.now());if(d.update("tasks",v,"id=?",new String[]{""+o.optLong("id")})==0)d.insertOrThrow("tasks",null,v);}d.setTransactionSuccessful();}finally{d.endTransaction();}}
...
public final class TaskActivity extends AppCompatActivity implements LocationListener{
 ...
 private void start(){if(!permissions()){db.log("warning","PERM_DENIED_START",permDetail());permissionGuide();return;}if(here==null){message.setText("请到室外等待卫星定位");return;}db.log("info","START_JOURNEY",permDetail());String shared=db.active(task.j.optLong("site_id"));if(shared!=null){activate(shared,false);return;}start.setEnabled(false);start.setText("正在锁定任务…");Location pos=new Location(here);if(!Util.online(this)){activate(db.journey(task.id,task.j.optLong("site_id"),pos,true),true);return;}work.execute(()->{String local=db.journey(task.id,task.j.optLong("site_id"),pos,false);try{JSONObject r=new Api(this).start(task.id,pos.getLatitude(),pos.getLongitude(),pos.getAccuracy());db.serverJourney(local,r.getJSONObject("journey").getLong("id"));runOnUiThread(()->activate(local,r.optBoolean("weakEvidence")));}catch(Api.ApiError e){db.abort(local);runOnUiThread(()->{start.setEnabled(true);start.setText("开始前往（记录轨迹）");message.setText(e.getMessage());});}catch(Exception e){db.offlineJourney(local);db.log("warning","OFFLINE_START "+e.getMessage());runOnUiThread(()->activate(local,true));}});}
 ...
 private void activate(String id,boolean weak){journey=id;prefs.journey(id);TrackingService.start(this,id);Toast.makeText(this,(distance()<300||weak?"轨迹记录已开始（起点距采样点不足300米或离线开始，将标记审核）":"轨迹记录已开始，每10秒保存位置。到达后扫码"),Toast.LENGTH_LONG).show();startActivity(new Intent(this,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP));finish();}
 private void scan(){if(journey==null){message.setText("请先开始前往");return;}if(here==null)return;if(distance()>task.j.optInt("severe_radius_m",300)){message.setText("超过300米，禁止开始采样");return;}scanner.launch(new Intent(this,ScanActivity.class));}
 ...
 private void ready(){if(distance()>task.j.optInt("normal_radius_m",30))chooseReason();else{photoButton.setVisibility(View.VISIBLE);message.setText(manual?"损坏标签放近处，后方拍实际环境":"二维码正确。装水后拍瓶子与现场环境");}}
 private void chooseReason(){new AlertDialog.Builder(this).setTitle(distance()<=task.j.optInt("exception_radius_m",80)?"30～80米：选择原因":"80～300米：严重可疑原因").setSingleChoiceItems(REASONS,-1,null).setPositiveButton("确定",(d,w)->{int x=((AlertDialog)d).getListView().getCheckedItemPosition();if(x<0){message.setText("必须选择原因");return;}reason=REASONS[x];photoButton.setVisibility(View.VISIBLE);message.setText("已记录原因："+reason);}).setNegativeButton("取消",null).show();}
  ...
  <dialog id="taskDialog">
    <form id="taskForm" method="dialog">
      ...
      <div id="taskFields" class="form-grid">
        ...
        <label class="wide">本次样品类型（对每个选中点位生效）
          <div class="type-checkboxes" id="taskTypes"></div>
        </label>
      </div>
...
SHA-256: `11980b4ec07e5683a771fafe88937873c2ab2aeea31895901206d3bbd3909c56`
