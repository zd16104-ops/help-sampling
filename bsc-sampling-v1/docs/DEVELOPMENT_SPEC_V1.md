...
- 自动化测试：`test/security.test.js`、`test/schema.test.js`、`test/api.test.js`、`test/backup.test.js`、`test/track.test.js` 共 52 项全部通过（npm test）；`test/smoke.js` 30 项端到端冒烟全部通过。
- Android 已完成本机完整编译（v1.1.0）：`assembleDebug`/`assembleRelease` 均成功（release 为正式签名，证书 SHA-256 `32178D6276B0536779A5676ED30623BB8CAFAFF09F328F3C642C05545F66D994`，keystore 与密码单独保管不入库）；CameraX 由 1.6.1 调整为 1.5.2（1.6.x 要求 compileSdk 36 + AGP 8.9.1，与本工程 compileSdk 35 + AGP 8.7.3 不兼容）；`QrDataTest` 单元测试 8/8 通过；构建工具链脚本 `tools/setup-toolchain.ps1` 入库。
- 管理站前端已全部切换到 `/api/v1`（`public/index.html`、`public/app.js`、`public/styles.css`）：管理员登录（密码+可选 TOTP）、项目与采样日期自动归档导航、待采样入口、卫星地图与状态色标记（灰/橙/绿/红，点位名称常显）、点位管理（地图选点+拖动微调、CSV 导入、编辑、参考图可选上传、启用开关）、任务下发（多点位×多类型批量生成、全选）、40 枚/页 A4 标签打印页（含打印次数记录）、审核详情（现场照片/参考图并排对比/轨迹/风险标志中文解释/审核意见追加，不修改原始记录）、**表格视图（筛选/搜索/批量审核/批量天气）**、任务取消/改期（重新编号）与设备解锁、服务器天气补齐、项目增删改查、导出（CSV/GeoJSON/照片包/审计/GPX）、设备激活二维码、诊断日志与磁盘健康提示；`/uploads/`、`/reference/` 图片已加签名鉴权，全站安全响应头（CSP 等）。Leaflet 1.9.4 与 qrcodejs 已本地托管于 `public/vendor/`，不依赖 CDN。无头浏览器端到端测试 `test/frontend.e2e.js`（Playwright）全部通过（断言数随数据量动态变化，覆盖登录/响应式/地图选点/审核/表格批量/标签/激活/日志等全流程）。
...
SHA-256: `49b0570855269049da72bc8d522a9370b690cd24b7d897fa398d572ee246445d`
