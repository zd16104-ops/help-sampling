...
- 移动端（Android APP）：`/api/v1/mobile/*` —— 激活、登录、同步、主/备用采样人任务、15 秒协作进度、在线接管、开始行程、轨迹批量上传、实时位置、幂等采样记录（照片 Base64 单包上传）、结束行程、诊断日志。
- 管理端：`/api/v1/admin/*` —— 登录（密码+可选 TOTP）、bootstrap、点位双语查询/新建/编辑（审计）、参考图上传压缩、设备激活二维码（自动或管理员设置 8 位密钥）、双人任务创建/查询/换人（按日期或待采样）、取消/解锁、已下发任务单个或最多 500 个批量补打图形化双语标签 PDF、协作进度/轨迹/证据合并审核、导出（CSV/GeoJSON/GPX/照片 ZIP/审计 CSV）、日志、健康检查（含磁盘余量）、天气补齐。
- 静态：管理站页面由 `/` 提供；照片 `/uploads/`、参考图 `/reference/`。

HTTP 语义：422 业务拒绝（超 300 m、二维码不匹配、异常原因缺失等），401/403 身份失败，409 采样权变化/接管冷却，429 登录/激活限速（5 次失败锁定 10 分钟窗口），413 过大。同一任务只有一个当前采样人；主、备用两人的历史轨迹和证据按任务合并，旧采样权证据保留但不能覆盖最终记录。移动端记录上传以 `client_record_id` 幂等。

## 测试与运维

```powershell
npm run check      # 全部 JS 语法检查
npm test           # 70 项自动化测试（安全、迁移、双人接管、证据合并、标签、备份、轨迹）
...
`public/index.html` + `public/app.js` 已全部对接 `/api/v1`：项目/拍摄日期导航、卫星地图状态色标记、点位双语管理（选点/CSV 导入/编辑/参考图）、主/备用双人下发（含项目默认与点位覆盖）、已下发标签补打、审核详情（两人照片/进度/分色轨迹合并、参考图、风险标志、审核意见）、取消/解锁、天气补齐、导出、设备激活二维码、诊断日志与磁盘健康。管理站功能图标和七种样本图标来自固定版本的 Material Symbols Rounded，并随 `public/icons.js` 本地提供；Leaflet 1.9.4 与 qrcodejs 同样本地托管在 `public/vendor/`，运行时不依赖 CDN。
